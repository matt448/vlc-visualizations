...
{
    RECT ball = {
        (int)(ball_x - (float)BALL_HALF_SIZE),
        (int)(ball_y - (float)BALL_HALF_SIZE),
        (int)(ball_x + (float)BALL_HALF_SIZE),
        (int)(ball_y + (float)BALL_HALF_SIZE)
    };

    return ball.right >= rect.left && ball.left <= rect.right &&
           ball.bottom >= rect.top && ball.top <= rect.bottom;
}

static int brick_collision_axis(float previous_x, float previous_y, RECT rect)
{
    if (previous_x + (float)BALL_HALF_SIZE <= (float)rect.left ||
        previous_x - (float)BALL_HALF_SIZE >= (float)rect.right)
        return 0;

    if (previous_y + (float)BALL_HALF_SIZE <= (float)rect.top ||
        previous_y - (float)BALL_HALF_SIZE >= (float)rect.bottom)
        return 1;
...
static void draw_ball(HDC dc, int cx, int cy, int half_size, float level)
{
...
static void breakout_draw(visualizer_sys_t *sys)
{
    ...
    HFONT score_font = CreateFontW(
        -15, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
        OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
        FIXED_PITCH | FF_MODERN, L"Consolas");
    HGDIOBJ old_score_font = SelectObject(sys->render_dc, score_font);
    wchar_t score_text[64];
    swprintf(score_text, ARRAYSIZE(score_text), L"SCORE %05u", score);
    RECT score_rc = { side_pad, 14, side_pad + 180, top - 6 };
    DrawTextW(sys->render_dc, score_text, -1, &score_rc,
...
vlc_module_begin()
    set_shortname("Breakout Advanced")
    set_description("Advanced Breakout visualization with song-long brick breaking")
    set_capability("visualization", 0)
    set_subcategory(SUBCAT_AUDIO_VISUAL)
    add_shortcut("breakout_advanced", "breakout_breaking_bricks")
